...
library(pheatmap)
setwd("...")             #设置工作目录
rt=read.table("riskAll.txt",sep="\t",header=T,row.names=1,check.names=F)     
...
data=read.table("riskAll.txt",header=T,sep="\t",check.names=F)
...
pdf(file=survFile,onefile = FALSE,width = 9,height =8)
print(surPlot)
dev.off()
